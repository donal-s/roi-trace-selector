...
it("SelectionIconView disabled initially", () => {
...
it("SelectionIconView change on item selection", () => {
  setCsvData(CSV_DATA);
...
it("SelectionIconView change state on click", () => {
  setCsvData(CSV_DATA);
  roiDataStore.dispatch({ type: SET_CURRENT_INDEX, index: 1 });
  checkPanel(false, false, true, false);
...
const selectionIconPanel = () => container.querySelector("#selectionIconPanel");
...
function checkPanel(isDisabled, unselectedActive, unscannedActive, selectedActive) {
  act(() => {
    render(
  ...
  expect(selectionIconPanel().className, "disabled flag").toBe(isDisabled ? "disabled" : "");
...
function checkButtonSelection(button, expectedSelected) {
    expect(classesContain(button.getAttribute("class"), "selected")).toBe(expectedSelected);
}
